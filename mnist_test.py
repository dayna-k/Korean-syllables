...
### built in functionality to handle the MNIST ###
...
def conv_layer(input, conv_kernel, conv_strides, conv_padding="SAME", name="Conv"):
    with tf.name_scope(name):
        # Convolution filter (kernel) parameters
        ...
        #Non linearly activation function
...
def pool_layer(input, pool_kernel, pool_strides, poo_padding="VALID", name="Pool"):
    with tf.name_scope(name):
        pool = tf.nn.max_pool(input, ksize=pool_kernel, strides=pool_strides, padding=poo_padding)
        return pool
#End of pool_layer
...
def CNN_model(x, n_classes, nShape, nchannels, dropout):
    ...
    pool2_shape = pool2.get_shape().as_list() # batch size, 7*7, ??
        ...
        for itr in range(2001):
            ...
            if itr & 10 == 0:
                [train_acc, s] = sess.run([Classification_accuracy, summ],
                                          feed_dict={ph_x: batch[0], ph_y: batch[1], keep_prob: 1.0})
                Train_writer.add_summary(s, itr)

                [test_acc, s] = sess.run([Classification_accuracy, summ],
                                         feed_dict={ph_x: mnist.test.images, ph_y: mnist.test.labels, keep_prob: 1.0})
